...
public class Controller : MonoBehaviour, ISceneController, IUserAction
{
    ...
    void Start()
    {
        ...
        Instantiate(Resources.Load("Prefabs/platform"), new Vector3(0,-10,-6), Quaternion.identity);
    ...
    public void send(int type){
        ...
        if (type == 1){     //蓝色飞盘分数为1，简单一点，速度慢，倾角大
            ran_y = Random.Range(1f,2f);
            power = Random.Range(2f,2.5f);
            ran_z = Random.Range(0f,1f);
            // angle = Random.Range(20f,30f);
        }
        else if (type == 2){       //红色飞盘分数为2，稍难一点，速度稍快，倾角适中
            ran_y = Random.Range(2f,3f);
            power = Random.Range(2.5f,3f);
            ran_z = Random.Range(1f,2f);
            // angle = Random.Range(10f,20f);
        }
        else if (type == 3){     //绿色飞盘分数为3，最难，速度最快，倾角最小
            ran_y = Random.Range(3f,4f);
            power = Random.Range(3f,4f);
            ran_z = Random.Range(2f,3f);
            // angle = Random.Range(0f,10f);
        }
        curDisk.transform.position = new Vector3(ran_x*16f, ran_y * 2, ran_z);
        action_manager.DiskFly(curDisk, power * 3);
    }
